...
#from keras.losses.losses_utils import compute_weighted_loss
...
class CosineLrSchedulerEpoch(keras.callbacks.Callback):
    def __init__(self, lr_base, first_restart_step, m_mul=0.5, t_mul=2.0, lr_min=1e-6, lr_warmup=-1, warmup_steps=0, cooldown_steps=1):
        ...
        if lr_min == lr_base * m_mul:
            self.schedule = tf.keras.optimizers.schedules.CosineDecay(lr_base, first_restart_step, alpha=lr_min / lr_base)
            self.cooldown_steps_start, self.cooldown_steps_end = np.array([]), np.array([])
        else:
            self.schedule = tf.keras.optimizers.schedules.CosineDecayRestarts(lr_base, first_restart_step, t_mul=t_mul, m_mul=m_mul, alpha=lr_min / lr_base)
...
class CosineLrScheduler(keras.callbacks.Callback):
    ...
    def build(self, steps_per_epoch=-1):
        ...
        if self.lr_min == self.lr_base * self.m_mul:  # Without restart
            self.schedule = tf.keras.optimizers.schedules.CosineDecay(self.lr_base, first_restart_batch_step, alpha=alpha)
            self.cooldown_steps_start, self.cooldown_steps_end = np.array([]), np.array([])
        else:
            self.schedule = tf.keras.optimizers.schedules.CosineDecayRestarts(self.lr_base, first_restart_batch_step, t_mul=self.t_mul, m_mul=self.m_mul, alpha=alpha)
    ...
    def on_train_batch_begin(self, iterNum, logs=None):
        ...
        if self.model is not None:
            ...
            if isinstance(opt, tf.keras.mixed_precision.LossScaleOptimizer):
                opt = opt.inner_optimizer  # Truy cập optimizer thực tế bên trong
            #print(opt.get_config())  # Xem thông tin optimizer
            opt.learning_rate = lr  # Hoặc `opt.learning_rate` nếu `opt.lr` không có
